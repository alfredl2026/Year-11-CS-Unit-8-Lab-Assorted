...
public class Assorted {
    ...
    public static List<Integer> subtractList(List<Integer> listA, List<Integer> listB) {
        ...
        for (Object a : listA) {
            if (!Objects.equals(listA.get(i), listB.get(0))) {
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for (int i = lowerBound; i <= upperBound; i++) {
            ...
            for (int j = 1; j <= String.valueOf(i).length(); j++) {
                value = (int) (value + Math.pow(Integer.parseInt(String.valueOf(i).charAt(j - 1) + ""), j));
            }
            if (value == i || value == 0) {
                newList.add(i);
            }
    ...
    public static int queueTime(List<Integer> queue, int tillsOpen) {
        ...
        for (int i = 0; i < queue.size(); i++) {
            int minTime = tills.get(0);
            int minIndex = 0;
            for (int j = 1; j < tills.size(); j++) {
                if (tills.get(j) < minTime) {
                    minTime = tills.get(j);
                    minIndex = j;
                }
            }
            tills.set(minIndex, tills.get(minIndex) + queue.get(i));
        }
        int maxTime = tills.get(0);
        for (int i = 1; i < tills.size(); i++) {
            if (tills.get(i) > maxTime) {
                maxTime = tills.get(i);
            }
        }
        return maxTime;
